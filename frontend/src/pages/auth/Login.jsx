/**
 * Login - Login page with mode awareness
 * - Redirects to /setup if no users exist (admin_only mode)
 * - Shows/hides register link and OAuth based on software_mode
 */
...
export default function Login() {
  ...
  const { login, isLoading: authLoading, error, isAuthenticated } = useAdminStore();
  ...
  // Check setup status and fetch config
  useEffect(() => {
    const checkStatus = async () => {
      try {
        // Check if setup is needed (admin_only mode only)
        ...
        // Fetch public config for mode awareness
  ...
  const isPlatformMode = config?.software_mode === "platform";
    ...
    <div className="min-h-screen flex items-center justify-center px-4 transition-colors duration-300">
      <div className="card max-w-md w-full">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-2">Willkommen zurück</p>
        </div>

        {/* Register link - only in platform mode */}
        {isPlatformMode && (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button type="submit" disabled={authLoading} className="btn-primary w-full flex items-center justify-center gap-2">
            {authLoading ? (
              <>
        ...
        {/* OAuth Section - only in platform mode with configured providers */}
        {isPlatformMode && oauthProviders.length > 0 && (
          <>
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-white dark:bg-slate-900 text-slate-500">oder</span>
              </div>
            </div>
