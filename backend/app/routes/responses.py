...
from .surveys import load_survey_definition as load_public_survey_definition
...
def extract_client_meta(request: Request, meta: dict) -> dict:
    ...
    if client_ip and settings.COLLECT_IP:
        enriched_meta["ip_hash"] = hashlib.sha256(
...
async def create_response(
    ...
    survey_def = await load_public_survey_definition(response_data.survey_id, db)
    ...
    asyncio.create_task(
        send_response_notification(
            survey_id=response_data.survey_id,
            response_id=str(db_response.id),
            survey_title=survey_def.get(
                "title", response_data.survey_id) if survey_def else response_data.survey_id,
